...
function iniciarModoViaje(destino, km, tiempo) {
  ...
  // Limpiar mapa — ocultar overlays que estorban
  var ocultar = ['gpsStatus','climaOverlay','panelPeligrosRuta'];
  ocultar.forEach(function(id) {
    var el = document.getElementById(id);
    if (el) el.style.display = 'none';
  });
  // Ocultar leyenda + botón reportar
  var leyenda = document.querySelector('.leyenda-mapa');
...
function terminarViaje() {
  ...
  // Restaurar overlays
  var st = document.getElementById('gpsStatus');
  if (st) { st.style.display = 'block'; st.textContent = '✅ Viaje terminado · Buen trabajo 🚛'; }
  var leyenda = document.querySelector('.leyenda-mapa');

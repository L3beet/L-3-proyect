...
function trazarRutaEnMapa(destLat, destLng, nombreDestino) {
  ...
  navigator.geolocation.getCurrentPosition(function(pos) {
    ...
    fetch('https://api.openrouteservice.org/v2/directions/driving-hgv/geojson', {
      ...
      body: JSON.stringify({
        coordinates: [[oLng, oLat], [destLng, destLat]],
        options: {
          vehicle_type: 'hgv',
          profile_params: {
    ...
    .then(function(data) {
      ...
      rutaLine = L.polyline(coords, { color: '#e50914', weight: 6, opacity: 0.9 }).addTo(mapInstance);
      ...
      st.innerHTML = '🚛 ' + km + ' km · ' + (horas > 0 ? horas + 'h ' : '') + mins + 'min · ' + (nombreDestino||'destino') +
        ' <span style="color:#f1c40f;font-size:9px;">· ruta HGV camión</span>';

      peligrosEnRuta(coords);
    })
